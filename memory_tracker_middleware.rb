...
class MemoryTrackerMiddleware
  ...
  def call(env)
    ...
    log_memory_threshold_exceeded(env, request_id, memory_after - memory_before) if memory_growth_exceeded?(memory_after, memory_before)
  ...
  def log_request(env, request_id, gc_stat_before, gc_stat_after, memory_before, memory_after, objects_before, objects_after, query_cache_before, query_cache_after, request_time)
    log_data = {
  ...
  def log_memory_threshold_exceeded(env, request_id, memory_growth)
    log_data = {
      request_id: request_id,
      method: env['REQUEST_METHOD'],
      url: env['REQUEST_URI'],
